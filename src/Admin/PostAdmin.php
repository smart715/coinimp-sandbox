...
class PostAdmin extends PostAdminBase
{
    ...
    {
        $this->logger->info('add_post', [$this->getPostUrl($object)]);
    ...
    {
        $this->logger->info('edit_post', [$this->getPostUrl($object)]);
    ...
    {
        $this->logger->info('delete_post', [$this->getPostUrl($object)]);
    ...
    public function preBatchAction($actionName, ProxyQueryInterface $query, array &$idx, $allElements): void
    {
        if ($allElements || count($idx) === $this->getPostsCount()) {
            $this->logger->info('delete_all_posts');
        } else {
